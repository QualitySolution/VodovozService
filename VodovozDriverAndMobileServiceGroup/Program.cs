...
using NLog;
using VodovozMobileService;
using VodovozAndroidDriverService;
using Nini.Config;
...
namespace VodovozDriverAndMobileServiceGroup
{
	class MainClass
	{
		...
		private static string mobileConfigFile = "/etc/vodovoz-mobile-service.conf";
		...
		public static void Main(string[] args)
		{
			...
			try {
				...
				//настройки базы считываются только с первого файла настройки, так как должны быть идентичными
				...
				IniConfigSource mobileConfFile = new IniConfigSource(mobileConfigFile);
				mobileConfFile.Reload();
				mobileServiceConfig = mobileConfFile.Configs["Service"];
			}
			catch(Exception ex) {
